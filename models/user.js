...
class User {
  ...
  static async register({ username, password, first_name, last_name, phone }) {
    let hasedPassword = await bcrypt.hash(password, BCRYPT_WORK_FACTOR);
  ...
  static async authenticate(username, password) {
    ...
    let userPassword = passwordInDb.rows[0].password;
  ...
  static async updateLoginTimestamp(username) {
    ...
    if (!result.rows[0]) {
      throw new ExpressError("Can't find the username", 404);
    }
  }
